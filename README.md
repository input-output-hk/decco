...
```bash
bazel build //src/io/iohk/codecs
```

Or, to run it's associated tests (that is the rule `tests`)

```bash
bazel test //src/io/iohk/codecs
```
...
```bash
bazel test //src/io/iohk/codecs --test_output=all
```

Or, if you are only interested on the tests that fail:

```bash
bazel test //src/io/iohk/codecs --test_output=errors
```
