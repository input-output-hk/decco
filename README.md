...
For example, to build `decco` you need to run this:

```bash
bazel build //main/io/iohk/codecs
```

Or, to run it's associated tests (that is the rule `tests`)

```bash
bazel test //main/io/iohk/codecs
```
...
```bash
bazel test //main/io/iohk/codecs --test_output=all
```

Or, if you are only interested on the tests that fail:

```bash
bazel test //main/io/iohk/codecs --test_output=errors
```
